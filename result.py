...
def createTransformedDataMeasuresSet(filename):
    with open(filename, 'rb') as f:
        array = np.fromfile(f, dtype = '>u2')
        minutesOfData = int(array.size / 2640)
        result = np.empty((minutesOfData*60, 43), dtype=np.uint32)
...
def createRawDataMeasuresSet(filename):
    with open(filename, 'rb') as f:
        ...
        for (j) in range(minutesOfData):
            result[j][0] = array[15+(j // 60)*2640+j*43] # Hour of day for {i+1} second of data
            result[j][1] = array[16+(j // 60)*2640+j*43] # Minute of hour for {i+1} second of data
            result[j][2] = array[17+(j // 60)*2640+j*43] #Second of minute for {i+1} second of data
            result[j][3] = array[21+(j // 60)*2640+j*43] #Chanel 1, 30000 eV electrons
            result[j][4] = array[20+(j // 60)*2640+j*43] #Chanel 2, 20400 eV electrons
            result[j][5] = array[19+(j // 60)*2640+j*43] #Chanel 3, 13900 eV electrons
            result[j][6] = array[18+(j // 60)*2640+j*43] #Chanel 4, 9450 eV electrons
            result[j][7] = array[25+(j // 60)*2640+j*43] #Chanel 5, 6460 eV electrons
            result[j][8] = array[24+(j // 60)*2640+j*43] #Chanel 6, 4400 eV electrons
            result[j][9] = array[23+(j // 60)*2640+j*43] #Chanel 7, 3000 eV electrons
            result[j][10] = array[22+(j // 60)*2640+j*43] #Chanel 8, 2040 eV electrons
            result[j][11] = array[29+(j // 60)*2640+j*43] #Chanel 9, 1392 eV electrons
            result[j][12] = array[28+(j // 60)*2640+j*43] #Chanel 10, 949 eV electrons 
            result[j][13] = array[27+(j // 60)*2640+j*43] #Chanel 11, 949 eV electrons
            result[j][14] = array[26+(j // 60)*2640+j*43] #Chanel 12, 646 eV electrons
            result[j][15] = array[33+(j // 60)*2640+j*43] #Chanel 13, 440 eV electrons
            result[j][16] = array[32+(j // 60)*2640+j*43] #Chanel 14, 300 eV electrons
            result[j][17] = array[31+(j // 60)*2640+j*43] #Chanel 15, 204 eV electrons
            result[j][18] = array[30+(j // 60)*2640+j*43] #Chanel 16, 139 eV electrons
            result[j][19] = array[37+(j // 60)*2640+j*43] #Chanel 17, 95 eV electrons
            result[j][20] = array[36+(j // 60)*2640+j*43] #Chanel 18, 65 eV electrons
            result[j][21] = array[35+(j // 60)*2640+j*43] #Chanel 19, 44 eV electrons
            result[j][22] = array[34+(j // 60)*2640+j*43] #Chanel 20, 30 eV electrons
            result[j][23] = array[41+(j // 60)*2640+j*43] #Chanel 1, 30000 eV ions
            result[j][24] = array[40+(j // 60)*2640+j*43] #Chanel 2, 20400 eV ions
            result[j][25] = array[39+(j // 60)*2640+j*43] #Chanel 3, 13900 eV ions
            result[j][26] = array[38+(j // 60)*2640+j*43] #Chanel 4, 9450 eV ions
            result[j][27] = array[45+(j // 60)*2640+j*43] #Chanel 5, 6460 eV ions
            result[j][28] = array[44+(j // 60)*2640+j*43] #Chanel 6, 4400 eV ions
            result[j][29] = array[43+(j // 60)*2640+j*43] #Chanel 7, 3000 eV ions
            result[j][30] = array[42+(j // 60)*2640+j*43] #Chanel 8, 2040 eV ions
            result[j][31] = array[49+(j // 60)*2640+j*43] #Chanel 9, 1392 eV ions
            result[j][32] = array[48+(j // 60)*2640+j*43] #Chanel 10, 949 eV ions
            result[j][33] = array[47+(j // 60)*2640+j*43] #Chanel 11, 949 eV ions
            result[j][34] = array[46+(j // 60)*2640+j*43] #Chanel 12, 646 eV ions
            result[j][35] = array[53+(j // 60)*2640+j*43] #Chanel 13, 440 eV ions
            result[j][36] = array[52+(j // 60)*2640+j*43] #Chanel 14, 300 eV ions
            result[j][37] = array[51+(j // 60)*2640+j*43] #Chanel 15, 204 eV ions
            result[j][38] = array[50+(j // 60)*2640+j*43] #Chanel 16, 139 eV ions
            result[j][39] = array[57+(j // 60)*2640+j*43] #Chanel 17, 95 eV ions
            result[j][40] = array[56+(j // 60)*2640+j*43] #Chanel 18, 65 eV ions
            result[j][41] = array[55+(j // 60)*2640+j*43] #Chanel 19, 44 eV ions
            result[j][42] = array[54+(j // 60)*2640+j*43] #Chanel 20, 30 eV ions
        del array
        #Тут все ок
        #TODO: возвращать Dataset
        return result
